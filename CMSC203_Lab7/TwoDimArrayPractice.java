   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   int minimumValue = 0;
	   for(int row = 0; row < intArray.length; row++) {
		   if(minimumValue == 0 || minimumValue >= intArray[row][column] )
	   ...
	   for(int row = 0; row < intArray.length; row++ ) {
		   for(int column = 0; column < intArray[row].length; column++) {
			   if(intArray[row][column] == value)
				   foundCount++;
			   animate(row, column, foundCount);
		   }
	   }

     return foundCount; // replace this line with your return statement
   }
